...
interface EvaluateStewardPageProps {
  params: Promise<{ id: string }>;
}

const EvaluateStewardPage = ({ params }: EvaluateStewardPageProps) => {
  const [stewardId, setStewardId] = React.useState<string>('');
  const router = useRouter();

  React.useEffect(() => {
    const getParams = async () => {
      const resolvedParams = await params;
      setStewardId(resolvedParams.id);
    };
    getParams();
  }, [params]);
